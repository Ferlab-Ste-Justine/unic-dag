"""
            curated_softpath_hl7_docling_backfill
-------------------------------------------------------------------------

Backfill pipeline that parsed the base64-encoded PDF documents stored in the
``observation_value_base64`` column of ``curated_softpath_hl7_oru_r01_obx`` with docling, each run
...
Ongoing daily parsing of this source runs as the last step of ``curated_softpath_hl7``.
...
INPUT_SOURCE_ID = "curated_softpath_hl7_oru_r01_obx"
REPORT_DELTA_DESTINATION_ID = "curated_softpath_hl7_oru_r01_obx_parsing_reports_delta"
TABLES_DESTINATION_ID = "curated_softpath_hl7_oru_r01_obx_parsing_tables"
REPORT_MD_DESTINATION_ID = "curated_softpath_hl7_oru_r01_obx_parsing_report_md"
...
dag = DAG(
    dag_id="curated_softpath_hl7_docling_backfill",
    params={
    ...
    doc_md=__doc__,
    start_date=pendulum.datetime(2026, 8, 29, 0, tz=LOCAL_TZ),
    end_date=pendulum.datetime(2026, 8, 28, 0, tz=LOCAL_TZ),
...
with dag:
    start_task = start("start_curated_softpath_hl7_docling_backfill")
    end_task = end("end_curated_softpath_hl7_docling_backfill")
